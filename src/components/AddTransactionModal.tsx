    ...
    <div className="fixed inset-0 flex items-center justify-center p-4 z-50 fade-in"
      ...
      <div className="rounded-2xl w-full max-w-md modal-enter"
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4"
          ...
          <h2 className="text-base font-semibold" style={{ color: 'var(--text-primary)' }}>Add Transaction</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div><label style={label}>Amount (₹)</label>
            <input type="number" step="0.01" value={formData.amount}
              ...
              required placeholder="0.00" className="dark-input w-full px-3 py-2.5 rounded-xl text-sm" /></div>
          ...
          <div><label style={label}>Category</label>
            <input type="text" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })}
              required placeholder="e.g. Food, Salary" className="dark-input w-full px-3 py-2.5 rounded-xl text-sm" /></div>
          <div><label style={label}>Description</label>
            <input type="text" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })}
              required placeholder="What was this for?" className="dark-input w-full px-3 py-2.5 rounded-xl text-sm" /></div>
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading}
              className="gold-btn flex-1 px-4 py-2.5 rounded-xl text-sm flex items-center justify-center gap-2 disabled:opacity-50">
